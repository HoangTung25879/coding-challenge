...
export function InlineEditField<T>({
  ...
  const display = displayValue ?? (value !== null && value !== undefined && String(value) !== '' ? String(value) : '—')
    ...
    return (
      <div className="flex items-center gap-2 group min-h-[28px] hover:bg-gray-50">
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 focus:opacity-100 p-0.5 rounded text-gray-400 hover:text-gray-600 transition-opacity"
        >
    ...
    <div className="space-y-1" onKeyDown={handleKeyDown}>
      <div className="flex items-center gap-2">
        <div className="flex-1">
          {renderInput({ value: localValue, onChange: setLocalValue })}
        </div>
        <button
          type="button"
          aria-label="Save"
