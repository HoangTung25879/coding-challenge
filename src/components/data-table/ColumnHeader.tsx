...
export function ColumnHeader({ header, isSticky, stickyOffset = 0 }: Props) {
  ...
  return (
    <th
      className={`relative px-3 py-2 text-left text-xs font-medium whitespace-nowrap text-gray-500 select-none bg-gray-50${isRightSticky && isSticky ? ' border-l border-gray-200' : ''}`}
      style={{
      ...
      aria-label={
        isSticky
          ? `${String(header.column.columnDef.header ?? header.column.id)} (pinned)`
          : undefined
      }
    ...
    >
      ...
      {header.column.getCanResize() && (
        <div
          ...
          className={`absolute top-0 right-0 h-full w-1 cursor-col-resize touch-none select-none hover:bg-blue-400 ${
