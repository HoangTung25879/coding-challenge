...
export function ColumnHeader({ header, isSticky, stickyOffset = 0, onToggleSticky }: Props) {
    ...
    >
      <div className="flex items-center gap-1">
        ...
        {onToggleSticky && (
          <button
            type="button"
            className="ml-auto text-gray-400 hover:text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity"
            onClick={() => onToggleSticky(header.column.id)}
            title={isSticky ? 'Unpin column' : 'Pin column'}
            aria-label={isSticky ? `Unpin ${String(header.column.columnDef.header)} column` : `Pin ${String(header.column.columnDef.header)} column`}
          >
