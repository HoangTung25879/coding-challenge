...
describe('GET /api/leads/:id/activities', () => {
  it('returns activities for a lead as { data: Activity[] }', async () => {
    const res = await fetch(`${BASE}/api/leads/lead-001/activities`)
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toHaveProperty('data')
    expect(Array.isArray(body.data)).toBe(true)
    expect(body.data.every((a: { leadId: string }) => a.leadId === 'lead-001')).toBe(true)
  })
  ...
  it('returns empty data array for lead with no activities', async () => {
    const res = await fetch(`${BASE}/api/leads/lead-022/activities`)
    const body = await res.json()
...
describe('POST /api/leads/:id/activities', () => {
  it('creates a new activity and returns 201', async () => {
    ...
    const body = await res.json()
    expect(body.leadId).toBe('lead-001')
  ...
  it('persists the new activity to the store', async () => {
    const countBefore = activitiesStore.filter(a => a.leadId === 'lead-001').length
    await fetch(`${BASE}/api/leads/lead-001/activities`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'email', subject: 'Sent quote', note: 'PDF attached', createdBy: 'salesperson-1' }),
    })
    const countAfter = activitiesStore.filter(a => a.leadId === 'lead-001').length
    expect(countAfter).toBe(countBefore + 1)
  })

  it('injects leadId from URL — ignores leadId in body', async () => {
    const res = await fetch(`${BASE}/api/leads/lead-002/activities`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'note', subject: 'Note', note: 'Test', createdBy: 'salesperson-1', leadId: 'WRONG' }),
    })
    const body = await res.json()
    expect(body.leadId).toBe('lead-002')
  })
  ...
  it('returns 400 for invalid activity type', async () => {
    const res = await fetch(`${BASE}/api/leads/lead-001/activities`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'meeting', subject: 'Meeting', note: 'Test', createdBy: 'salesperson-1' }),
    })
    expect(res.status).toBe(400)
  })
  ...
  it('returns 404 for unknown lead', async () => {
    const res = await fetch(`${BASE}/api/leads/does-not-exist/activities`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'call', subject: 'Test', note: 'Test', createdBy: 'salesperson-1' }),
    })
